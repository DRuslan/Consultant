        ...
        fieldRules.forEach((rule) => {
          ...
          if (!RULES[ruleName](fieldCurrent.value, ...ruleParams)) {
            ...
            const formattedErrorMsg = errorMsg
              .replace(/\{(\d+)\}/g, (match, index) => ruleParams[index] || match)
...
//                     const l = 'A-Za-z0-9';
//                     const localPart = "([.]{0,1}[-!#$%&\'*+\/=?^_`{|}~A-Za-z0-9]){1,}";
//                     const domain = `[${l}]([-]{0,1}[${l}])*(\.[-${l}]*[${l}]){1,}`;
//                     return new RegExp(`^${localPart}@${domain}$`).test(value);
//                 },
//             };
//             // Метод для проведения валидации значения полей
//             const form = this.form

//             Object.keys(form).forEach((item) => {
//                 if (!Object.hasOwn(form[item], 'rules')) {
//                     return;
//                 }
